...
class JustJoinIt():
    ...
    def getJobsLinkList(self, url):
        ...
        return self.job_links_list

    def updateJobsDict(self, job_links_list):
        try:
            for job in job_links_list:
                job_link = "https://"+self.domainName+job.find('a',  class_='css-4lqp8g')['href']
...
def run(sheetname, url): # pragma: no cover
    ...
    just = JustJoinIt()
    jobs_list = just.getJobsLinkList(url)
    jobs_dict = just.updateJobsDict(jobs_list)
