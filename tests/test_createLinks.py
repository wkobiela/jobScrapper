...
def test_createLinks_not_enough_arguments():
    with pytest.raises(SystemExit):
        assert common.createLinks(role="testing", lvl="junior,mid", city="Gdańsk")
        
def test_createLinks_wrong_arguments():
    with pytest.raises(SystemExit):
        assert common.createLinks(website='NoFluffJobs', role="testing", lvl="junior,mid", city="Gdańsk")
        
def test_createLinks_too_much_arguments():
    with pytest.raises(SystemExit):
        assert common.createLinks(website='NoFluffJobs', role="testing", lvl="junior,mid", city="Gdańsk", more="Somemore")
